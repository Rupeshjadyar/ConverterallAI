...
@Component({ selector: 'app-excel-to-pdf', standalone: true, imports: [CommonModule, FormsModule, RouterModule], templateUrl: './excel-to-pdf.component.html', styleUrls: ['./excel-to-pdf.component.css'] })
export class ExcelToPdfComponent implements OnInit {
  ...
  ngOnInit() { this.title.setTitle('Excel to PDF – Convert XLSX/CSV to PDF | ConverterAllAI'); this.meta.updateTag({ name: 'description', content: 'Convert spreadsheets to PDF. Supports CSV files.' }); }
  ...
  async processFiles() {
    ...
    try {
      ...
      const margin = 50; const pageW = 841.89; const pageH = 595.28; // landscape A4
      const fontSize = 8; const lineH = fontSize * 1.6;
      ...
      for (let i = 0; i < lines.length; i += lpp) {
        ...
        for (const line of lines.slice(i, i+lpp)) {
          const trimmed = line.substring(0, 120); // limit width
          if (trimmed) page.drawText(trimmed, { x: margin, y, size: fontSize, font, color: rgb(0,0,0) });
          y -= lineH;
        }
      }
      this.progress = 90;
  ...
  download() { if (!this.convertedBlob) return; const a = document.createElement('a'); a.href = URL.createObjectURL(this.convertedBlob); a.download = (this.selectedFiles[0]?.name.split('.')[0] || 'spreadsheet') + '.pdf'; a.click(); }
